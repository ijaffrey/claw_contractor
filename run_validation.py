...
def run_all_validations():
    ...
    try:
        result = subprocess.run(['python3', 'validate_coverage.py'], 
                              capture_output=True, text=True)
        if result.returncode == 0:
            coverage_data = json.loads(result.stdout.split('\n')[0])
            validation_report['coverage_validation'] = coverage_data
            validation_report['coverage_validation']['status'] = 'PASS' if coverage_data['coverage_percentage'] >= 80 else 'FAIL'
        else:
    ...
    try:
        result = subprocess.run(['python3', 'validate_quality.py'], 
                              capture_output=True, text=True)
        if result.returncode == 0:
            quality_data = json.loads(result.stdout.split('\n')[0])
            validation_report['quality_validation'] = quality_data
            validation_report['quality_validation']['status'] = 'PASS' if quality_data['overall_pass'] else 'FAIL'
        else:
    ...
    # Run basic tests
    try:
        result = subprocess.run(['python3', 'run_basic_tests.py'], 
                              capture_output=True, text=True)
        validation_report['test_execution']['status'] = 'PASS' if result.returncode == 0 else 'FAIL'
