...
class ConversationState(ConversationBase):
    ...
    lead_id = Column(Integer, nullable=True, index=True)  # No FK constraint to avoid circular dependency
    ...
    def add_answer(self, answer: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error adding answer to conversation {self.thread_id}: {str(e)}")
            return False
    ...
    def set_questions(self, questions: List[str]) -> bool:
        ...
        try:
            ...
            logger.error(f"Error setting questions for conversation {self.thread_id}: {str(e)}")
            return False
