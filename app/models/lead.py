...
class Lead(Base):
    ...
    attachments = relationship(
        "LeadAttachment", back_populates="lead", cascade="all, delete-orphan"
    )
    ...
    def __repr__(self):
        return (
            f"<Lead(id={self.id}, email='{self.email}', status='{self.status.value}')>"
        )
    ...
    def is_active_opportunity(self):
        ...
        return (
            self.status
            in [LeadStatus.QUALIFIED, LeadStatus.PROPOSAL, LeadStatus.NEGOTIATION]
            and self.is_active
        )
