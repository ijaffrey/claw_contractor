...
class NotificationService:
    ...
    def send_contractor_notification(self, lead_id: int) -> Dict[str, Any]:
        ...
        """
        try:
            lead = Lead.objects.select_related("contractor", "customer").get(id=lead_id)
            ...
            if not lead.contractor.email:
                raise ValueError(
                    f"Contractor {lead.contractor.id} has no email address"
                )

            # Prepare email context
            context = {
                "contractor_name": lead.contractor.company_name
                or lead.contractor.user.get_full_name(),
            ...
            html_message = render_to_string(
                "emails/contractor_notification.html", context
            )
            plain_message = render_to_string(
                "emails/contractor_notification.txt", context
            )
            ...
            if success:
                ...
                logger.info(
                    f"Contractor notification sent successfully for lead {lead_id}"
                )
                return {
                    "success": True,
                ...
                logger.error(
                    f"Failed to send contractor notification for lead {lead_id}"
                )
                return {
                    "success": False,
            ...
            error_msg = (
                f"Error sending contractor notification for lead {lead_id}: {str(e)}"
            )
            ...
            try:
                recipient = (
                    ...
                    if "lead" in locals() and lead.contractor
                    else "unknown"
                )
                self.log_notification(
                    notification_type=NotificationType.CONTRACTOR_NOTIFICATION,
    ...
    def send_customer_handoff(self, lead_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            html_message = render_to_string("emails/customer_handoff.html", context)
            plain_message = render_to_string("emails/customer_handoff.txt", context)
            ...
            if success:
                ...
                logger.info(
                    f"Customer handoff notification sent successfully for lead {lead_id}"
                )
                return {
                    "success": True,
                ...
                logger.error(
                    f"Failed to send customer handoff notification for lead {lead_id}"
                )
                return {
                    "success": False,
            ...
            try:
                recipient = (
                    lead.customer.email
                    if "lead" in locals() and lead.customer
                    else "unknown"
                )
                self.log_notification(
                    notification_type=NotificationType.CUSTOMER_HANDOFF,
    ...
    def send_system_alert(
        ...
        try:
            ...
            html_message = render_to_string("emails/system_alert.html", context)
            plain_message = render_to_string("emails/system_alert.txt", context)
            ...
            return {
                ...
                "message": (
                    ...
                    if success
                    else "Failed to send system alert"
                ),
                "recipient": recipient,
            }
    ...
    def get_notification_history(
        ...
        try:
            ...
            logger.error(
                f"Error retrieving notification history: {str(e)}", exc_info=True
            )
            return []
