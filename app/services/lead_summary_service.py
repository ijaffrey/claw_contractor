...
class LeadSummaryService:
    ...
    def _format_lead_details(self, lead: Lead) -> Dict[str, Any]:
        ...
        return {
            ...
            "estimated_value": (
                float(lead.estimated_value) if lead.estimated_value else None
            ),
            ...
            "follow_up_date": (
                lead.follow_up_date.isoformat() if lead.follow_up_date else None
            ),
        }
    ...
    def _format_qualification_data(self, db, lead: Lead) -> Dict[str, Any]:
        ...
        try:
            ...
            responses = (
                db.query(QualificationResponse)
            ...
            if question_ids:
                questions = (
                    db.query(QualificationQuestion)
            ...
            return {
                "status": (
                    "completed" if completion_percentage == 100 else "in_progress"
                ),
                "completion_percentage": completion_percentage,
                "responses": sorted(
                    formatted_responses, key=lambda x: x["question_id"]
                ),
    ...
    def _format_photo_gallery(self, db, lead: Lead) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "categories": list(
                    set(
                        p.get("category") for p in formatted_photos if p.get("category")
                    )
                ),
            }
            ...
            return {"total_photos": 0, "photos": [], "error": str(e)}
    ...
    def _calculate_summary_stats(self, db, lead: Lead) -> Dict[str, Any]:
        ...
        try:
            ...
            # Count photos
            photo_count = (
                db.query(PhotoUpload).filter(PhotoUpload.lead_id == lead.id).count()
            )
            ...
            return {
                ...
                "completion_status": self._get_completion_status(
                    lead, response_count, photo_count
                ),
            }
    ...
    def _get_completion_status(
        self, lead: Lead, response_count: int, photo_count: int
    ) -> str:
    ...
    def export_lead_summary(
        self, lead_id: int, format_type: str = "json"
    ) -> Dict[str, Any]:
        """
