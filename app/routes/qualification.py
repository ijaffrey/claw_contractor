...
def qualify_lead(lead_id):
    ...
    try:
        ...
        logger.info(
            f"Lead {lead_id} qualified by user {current_user_id} - Result: {'QUALIFIED' if data['qualified'] else 'UNQUALIFIED'}"
        )
            ...
            jsonify(
                {
                    ...
                    "lead": {
                        ...
                        "qualification_date": (
                            lead.qualification_date.isoformat()
                            if lead.qualification_date
                            else None
                        ),
...
def complete_qualification(lead_id):
    ...
    try:
        ...
        if lead.status not in [LeadStatus.QUALIFIED, LeadStatus.UNQUALIFIED]:
            return (
                jsonify(
                    {"error": "Lead must be qualified or unqualified before completion"}
                ),
                400,
            )
        ...
        if lead.qualified:
            ...
            for contractor in contractors:
                try:
                    notification_result = (
                        notification_service.send_contractor_notification(
                    ...
                    if notification_result.get("success"):
                        ...
                        logger.info(
                            f"Contractor notification sent to {contractor.email} for lead {lead_id}"
                        )

                except Exception as e:
                    logger.error(
                        f"Failed to send contractor notification to {contractor.email}: {str(e)}"
                    )
                    notifications_sent.append(
                        {
                            "contractor_id": contractor.id,
            ...
            try:
                ...
                logger.error(
                    f"Failed to send customer handoff for qualified lead {lead_id}: {str(e)}"
                )
                notifications_sent.append(
                    {
                        "recipient": lead.email,
            ...
            try:
                ...
                logger.error(
                    f"Failed to send customer handoff for unqualified lead {lead_id}: {str(e)}"
                )
                notifications_sent.append(
                    {
                        "recipient": lead.email,
        ...
        logger.info(
            f"Lead {lead_id} completed by user {current_user_id} - "
            ...
            jsonify(
                {
                    ...
                    "notifications": {
                        "total_sent": len(
                            [n for n in notifications_sent if n.get("status") == "sent"]
                        ),
                        "total_failed": len(
                            [
                                n
        ...
        db.session.rollback()
        logger.error(
            f"Database error completing qualification for lead {lead_id}: {str(e)}"
        )
        return jsonify({"error": "Database error occurred"}), 500
    except Exception as e:
        db.session.rollback()
...
def get_qualification_status(lead_id):
    ...
    try:
        ...
        qualification_status = {
            ...
            "qualified": lead.qualified,
            "qualification_date": (
                lead.qualification_date.isoformat() if lead.qualification_date else None
            ),
            "completion_date": (
                lead.completion_date.isoformat() if lead.completion_date else None
            ),
            "qualification_notes": lead.qualification_notes,
        ...
        notifications = (
            Notification.query.filter(Notification.data.contains({"lead_id": lead_id}))
...
def get_qualification_stats():
    ...
    try:
        ...
        recent_qualifications = (
            Lead.query.filter(Lead.qualification_date.isnot(None))
            .order_by(Lead.qualification_date.desc())
            .limit(10)
            .all()
        )
