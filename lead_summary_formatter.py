...
class LeadSummaryFormatter:
    ...
    def format_photo_analysis(self, analysis: PhotoAnalysis) -> str:
        ...
        if analysis.damage_severity:
            formatted_analysis.append(f"Damage severity: {analysis.damage_severity.title()}")
        
        if analysis.estimated_repair_type:
            formatted_analysis.append(f"Estimated repair type: {analysis.estimated_repair_type}")
        ...
        if analysis.analysis_results:
            ...
            for i, result in enumerate(analysis.analysis_results, 1):
                ...
                if 'damage_detected' in result:
                    formatted_analysis.append(f"  Damage detected: {', '.join(result['damage_detected'])}")
                if 'materials_identified' in result:
                    formatted_analysis.append(f"  Materials: {', '.join(result['materials_identified'])}")
    ...
    def format_conversation_summary(self, summary: ConversationSummary) -> str:
        ...
        if summary.customer_sentiment:
            formatted_summary.append(f"Customer sentiment: {summary.customer_sentiment.title()}")
    ...
    def format_contact_details(self, contact: ContactInfo) -> str:
        ...
        if contact.preferred_contact_method:
            formatted_contact.append(f"Preferred contact method: {contact.preferred_contact_method}")
    ...
    def format_problem_diagnosis(self, diagnosis: ProblemDiagnosis) -> str:
        ...
        formatted_diagnosis.append(f"Estimated complexity: {diagnosis.estimated_complexity}")
        ...
        if diagnosis.timeline_estimate:
            formatted_diagnosis.append(f"Estimated timeline: {diagnosis.timeline_estimate}")
    ...
    def generate_contractor_notification(
        ...
        notification.append(f"Timestamp: {datetime.now(timezone.utc).strftime(self.timestamp_format)}")
        notification.append("")
        ...
        if diagnosis.recommended_services:
            services_str = ", ".join(diagnosis.recommended_services[:3])  # Limit to top 3
        ...
        if photo_analysis and photo_analysis.photo_count > 0:
            notification.append(f"📷 {photo_analysis.photo_count} photos available for review")
    ...
    def generate_customer_handoff(
        ...
        handoff.append("")
        handoff.append("Great news! We've reviewed your service request and have the perfect contractor for your project.")
        handoff.append("")
        ...
        if contractor_info:
            ...
            handoff.append(f"Company: {contractor_info.get('company_name', 'Professional Service Provider')}")
            if 'rating' in contractor_info:
                handoff.append(f"Rating: {contractor_info['rating']} ⭐")
            if 'years_experience' in contractor_info:
                handoff.append(f"Experience: {contractor_info['years_experience']} years")
            handoff.append("")
        ...
        if diagnosis.urgency_level in [UrgencyLevel.HIGH, UrgencyLevel.EMERGENCY]:
            ...
            handoff.append("We've marked this as priority due to the urgent nature of your request.")
            handoff.append("")
    ...
    def generate_full_summary(
        ...
        summary_parts.append("")
        summary_parts.append("=" * 60)
        summary_parts.append(f"Summary generated: {datetime.now(timezone.utc).strftime(self.timestamp_format)}")
    ...
    def generate_brief_summary(
        ...
        brief.append(f"Customer: {contact.name} | {self._format_phone_number(contact.phone)}")
    ...
    def _categorize_responses(self, responses: Dict[str, Any]) -> Dict[str, List[Tuple[str, Any]]]:
        ...
        problem_keywords = ['issue', 'problem', 'damage', 'repair', 'broken', 'leak', 'emergency']
...
def format_multiple_leads(
    ...
    summaries.append(f"Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S %Z')}")
    summaries.append("")
