...
class QualificationEngine:
    ...
    def _validate_contractor_data(self, data: Dict[str, Any]) -> None:
        ...
        for field in required_fields:
            if field not in data:
                logger.warning(
                    f"Missing field '{field}' in contractor data, using default value"
                )
    ...
    def _calculate_completion_score(
        self, completion_rate: float, total_projects: int
    ) -> float:
        """
    ...
    def _calculate_rating_score(
        self, average_rating: float, total_projects: int
    ) -> float:
        """
        ...
        if isinstance(last_activity_date, str):
            try:
                last_activity_date = datetime.fromisoformat(
                    last_activity_date.replace("Z", "+00:00")
                )
            except ValueError:
