...
class Agents:
    ...
    def run_acceptance_criteria(self, criteria: str, timeout: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not commands:
                ...
                results['errors'].append('No shell commands found in acceptance criteria')
                return results
            ...
            for i, command in enumerate(commands):
                ...
                if command_result['success']:
                    ...
                    results['errors'].append(f"Command {i+1} failed: {command_result['error']}")
                    
            # Set overall status
            if results['failed_commands'] > 0:
                results['status'] = 'partial_failure' if results['successful_commands'] > 0 else 'failure'
    ...
    def _parse_shell_commands(self, criteria: str) -> List[str]:
        ...
        for match in matches:
            ...
            if any(keyword in match.lower() for keyword in ['ls', 'cd', 'cat', 'grep', 'find', 'echo', 'mkdir', 'rm', 'cp', 'mv', 'chmod', 'chown', 'ps', 'kill', 'curl', 'wget', 'git']):
                commands.append(match.strip())
    ...
    def validate_criteria(self, criteria: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not commands:
                validation['warnings'].append('No shell commands found in acceptance criteria')
