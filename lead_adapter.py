...
def normalize_lead(flat_lead: Dict[str, Any]) -> Dict[str, Any]:
    ...
    urgency_level = map_urgency(
        flat_lead.get("job_type", ""), flat_lead.get("urgency", "")
    )
...
def parse_location_string(location_str: str) -> Dict[str, str]:
    ...
    if len(parts) >= 2:
        ...
        if any(
            ...
            for word in ["st", "ave", "rd", "blvd", "dr", "ct", "ln"]
        ):
            result["address"] = parts[0]
        else:
            result["city"] = parts[0]
...
def format_photos(attachments: List[Any]) -> List[Dict[str, Any]]:
    ...
    for attachment in attachments:
        ...
        if isinstance(attachment, dict):
            ...
            photo_entry["filename"] = attachment.get("filename", "") or attachment.get(
                "name", ""
            )
            photo_entry["size"] = attachment.get("size")
            photo_entry["type"] = attachment.get("type", "") or attachment.get(
                "mime_type", ""
            )
