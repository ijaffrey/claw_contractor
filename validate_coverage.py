...
def analyze_test_coverage():
    ...
    source_files = [
        ...
        if not f.name.startswith("test") and f.name != "validate_coverage.py"
    ]
    ...
    for source_file in source_files:
        ...
        has_test = any(
            f"test_{module_name}" in tf.name or f"{module_name}_test" in tf.name
    ...
    if coverage_data["total_source_files"] > 0:
        coverage_data["coverage_percentage"] = (
            len(coverage_data["tested_modules"]) / coverage_data["total_source_files"]
        ) * 100
