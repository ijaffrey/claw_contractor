...
class LeadHandoffService:
    ...
    def is_lead_qualified(self, lead: Lead) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Error qualifying lead {lead.id if lead else 'None'}: {str(e)}"
            )
            return False
    ...
    def process_lead_handoff(self, lead: Lead) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.is_lead_qualified(lead):
                raise LeadHandoffError(
                    f"Lead {lead.id} does not meet qualification criteria"
                )
    ...
    def _check_engagement_level(self, lead: Lead) -> bool:
        ...
        if not hasattr(lead, "engagement_actions") or not lead.engagement_actions:
            return False
        return (
            len(lead.engagement_actions)
            >= self.qualification_criteria.required_engagement_actions
        )
    ...
    def _check_contact_attempts(self, lead: Lead) -> bool:
        ...
        if not hasattr(lead, "contact_attempts") or not lead.contact_attempts:
            return False
        return (
            len(lead.contact_attempts)
            >= self.qualification_criteria.required_contact_attempts
        )
    ...
    def _calculate_qualification_score(self, lead: Lead) -> float:
        ...
        if hasattr(lead, "engagement_actions") and lead.engagement_actions:
            engagement_score = min(
                len(lead.engagement_actions) * 5, 25
            )  # Max 25 points
        ...
        data_score = (
            completed_fields / len(self.qualification_criteria.required_fields)
        ) * 20  # Max 20 points
        score += data_score
    ...
    def _get_last_engagement_date(self, lead: Lead) -> Optional[str]:
        ...
        try:
            last_action = max(
                lead.engagement_actions, key=lambda x: x.get("timestamp", datetime.min)
            )
            return (
                last_action.get("timestamp", "").isoformat()
                if hasattr(last_action.get("timestamp", ""), "isoformat")
                else str(last_action.get("timestamp", ""))
            )
        except (ValueError, AttributeError):
            return None

    def _update_lead_handoff_metadata(
        self, lead_id: str, handoff_data: Dict[str, Any]
    ) -> None:
    ...
    def _send_handoff_notifications(
        self, lead: Lead, handoff_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            # Notify sales team
            sales_notification = (
                self.notification_service.send_lead_handoff_notification(
                    lead=lead, handoff_data=handoff_data, recipients=["sales-team"]
                )
            )
            ...
            if hasattr(lead, "owner_id") and lead.owner_id:
                owner_notification = (
                    self.notification_service.send_personal_notification(
            ...
            self.logger.error(
                f"Error sending handoff notifications for lead {lead.id}: {str(e)}"
            )
            notifications.append(
                {
                    "type": "error",
    ...
    def get_handoff_statistics(
        ...
        try:
            ...
            return {"total_handoffs": 0, "error": str(e)}
