...
class AttachmentConfig:
    ...
    def __post_init__(self):
        ...
        self.allowed_extensions = [
            ...
            if ext.lower()
            not in [blocked.lower() for blocked in self.blocked_extensions]
        ]
...
class NotificationRecipient:
    ...
    role: str = "user"
    active: bool = True
    priorities: List[EmailPriority] = field(
        default_factory=lambda: [EmailPriority.NORMAL]
    )
...
class EmailConfig:
    ...
    def __init__(self, environment: Optional[Union[str, Environment]] = None):
        ...
        self.sender_email = self._get_env(
            "EMAIL_SENDER_EMAIL", "noreply@clawcontractor.com"
        )
        self.reply_to = self._get_env("EMAIL_REPLY_TO", None)
        self.bounce_email = self._get_env(
            "EMAIL_BOUNCE_ADDRESS", "bounce@clawcontractor.com"
        )
        ...
        self.enable_contractor_notifications = self._get_bool(
            "EMAIL_ENABLE_CONTRACTOR_NOTIFICATIONS", True
        )
    ...
    def _load_retry_policy(self) -> RetryPolicy:
        ...
        return RetryPolicy(
            ...
            retry_on_connection_error=self._get_bool(
                "EMAIL_RETRY_ON_CONNECTION_ERROR", True
            ),
        )
    ...
    def _load_attachment_config(self) -> AttachmentConfig:
        ...
        return AttachmentConfig(
            ...
            temp_storage_path=self._get_env(
                "EMAIL_ATTACHMENT_TEMP_PATH", "/tmp/email_attachments"
            ),
    ...
    def _load_template_config(self) -> TemplateConfig:
        ...
        return TemplateConfig(
            ...
            supported_languages=self._get_list(
                "EMAIL_TEMPLATE_SUPPORTED_LANGUAGES", ["en", "es", "fr"]
            ),
            cache_templates=self._get_bool("EMAIL_TEMPLATE_CACHE", True),
            template_extension=self._get_env("EMAIL_TEMPLATE_EXTENSION", ".html"),
            contractor_notification=self._get_env(
                "EMAIL_TEMPLATE_CONTRACTOR_NOTIFICATION", "contractor_notification.html"
            ),
            project_update=self._get_env(
                "EMAIL_TEMPLATE_PROJECT_UPDATE", "project_update.html"
            ),
            payment_reminder=self._get_env(
                "EMAIL_TEMPLATE_PAYMENT_REMINDER", "payment_reminder.html"
            ),
            system_alert=self._get_env(
                "EMAIL_TEMPLATE_SYSTEM_ALERT", "system_alert.html"
            ),
            welcome=self._get_env("EMAIL_TEMPLATE_WELCOME", "welcome.html"),
            password_reset=self._get_env(
        ...
        if self.environment == Environment.DEVELOPMENT:
            ...
            for category in recipients.keys():
                if not recipients[category]:
                    recipients[category].append(
                        NotificationRecipient(
                            email=dev_email, name="Developer", role="developer"
                        )
                    )
    ...
    def _validate_configuration(self):
        ...
        total_recipients = sum(
            len(recipients) for recipients in self.recipients.values()
        )
        ...
        if not template_path.exists() and self.environment == Environment.PRODUCTION:
            logger.warning(
                f"Template directory does not exist: {self.templates.base_path}"
            )
        ...
        logger.info(
            f"Email configuration loaded successfully for {self.environment.value} environment"
        )

    def get_recipients_by_priority(
        self, priority: EmailPriority
    ...
    def is_attachment_allowed(self, filename: str, size_mb: float) -> bool:
        ...
        if self.attachments.allowed_extensions and file_ext not in [
            ext.lower() for ext in self.attachments.allowed_extensions
        ]:
            return False
    ...
    def get_template_path(self, template_name: str, language: str = None) -> str:
        ...
        template_file = getattr(
            self.templates,
            template_name,
            f"{template_name}{self.templates.template_extension}",
        )
...
def get_email_config(
    ...
    if _email_config is None or (
        environment and _email_config.environment != environment
    ):
