...
def generate_proposal(lead: dict, brand: str = "skipp") -> dict:
    ...
    if brand == "skipp":
        ...
        if permit_number:
            subject = SKIPP_SUBJECTS["permit_hook"].format(
                permit_number=permit_number, borough=borough
            )
        else:
    ...
    proposal_id = (
        f"{brand}-{lead.get('id', 'x')}-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
    )
...
def test_proposals():
    ...
    for lead in sample_leads:
        ...
        assert (
            lead["company"] in skipp["body"] or "your team" in skipp["body"]
        ), "Company missing from body"
        assert (
            lead["borough"] in skipp["body"]
        ), f"Borough missing from body: {skipp['body'][:100]}"
    ...
    assert (
        "320250001" in p["subject"] or "plumbing" in p["subject"].lower()
    ), "Permit hook not in subject"

    print(
        "✓ test_proposals passed — skipp + drafted proposals generated for all trade types"
    )
    return True
