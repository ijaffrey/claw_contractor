...
"""
        ...
        try:
            ...
            logger.info(
                f"Successfully wrote guidance for job {job_id} to {guidance_file}"
            )
            return True
...
def generate_sprint_plan(
    brief: str, repo: str, duration_days: int = 14
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Successfully generated sprint plan {sprint_id} with {len(tasks)} tasks"
        )
        return plan_dict
...
def generate_and_run(
    command: Optional[str] = None, config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Execution {result['execution_id']} completed with status: {result['status']}"
        )
...
__description__ = (
    "Production-quality MCP server for development guidance and automation"
)
