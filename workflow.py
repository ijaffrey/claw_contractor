...
class LeadHandoffWorkflow:
    ...
    def _send_contractor_notification(
    ...
    def _send_customer_handoff_message(
        self, lead_data: Dict[str, Any]
    ) -> WorkflowAction:
...
Customer Success Team
        ...
        for action in reversed(actions):
            try:
                ...
                rollback_errors.append(
                    f"Failed to rollback {action.action_type.value}: {str(e)}"
                )
                self.logger.error(
                    f"Rollback error for {action.action_type.value}: {str(e)}"
                )
    ...
    def complete_qualification(self, lead_id: str) -> WorkflowResult:
        ...
        workflow_result = WorkflowResult(
            workflow_id=workflow_id, lead_id=lead_id, status=WorkflowStatus.PENDING
        )
        ...
        try:
            ...
            self.logger.warning(
                f"Workflow validation failed for lead {lead_id}: {str(e)}"
            )
            raise e
            ...
            if workflow_result.actions_performed:
                self.logger.info(
                    f"Attempting rollback for {len(workflow_result.actions_performed)} actions"
                )
                rollback_errors = self._rollback_actions(
                    workflow_result.actions_performed, lead_id
                )

                if rollback_errors:
                    workflow_result.status = WorkflowStatus.ROLLED_BACK
                    workflow_result.error_details += (
                        f" | Rollback errors: {'; '.join(rollback_errors)}"
                    )
                else:
                    workflow_result.status = WorkflowStatus.ROLLED_BACK
                    self.logger.info(
                        f"Successful rollback completed for lead {lead_id}"
                    )
