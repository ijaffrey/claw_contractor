...
class NotificationManager:
    ...
    def detect_qualified_lead(self, lead_id: str) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if not lead_data.get('customer_name') or not lead_data['customer_name'].strip():
                missing_requirements.append("Customer name")
            
            if not lead_data.get('customer_email') or not self._is_valid_email(lead_data['customer_email']):
                missing_requirements.append("Valid customer email")
            
            if not lead_data.get('customer_phone') or not lead_data['customer_phone'].strip():
                missing_requirements.append("Customer phone number")
            
            # Check problem description
            if not lead_data.get('problem_description') or len(lead_data['problem_description'].strip()) < 20:
                missing_requirements.append("Detailed problem description (minimum 20 characters)")
            ...
            if not lead_data.get('budget_range') or lead_data['budget_range'] == 'not_specified':
            ...
            if qualification_score < 70:  # Minimum 70% qualification score
                missing_requirements.append(f"Qualification score too low ({qualification_score}% - minimum 70% required)")
            
            is_qualified = len(missing_requirements) == 0
            
            self.logger.info(f"Lead {lead_id} qualification check: {'QUALIFIED' if is_qualified else 'NOT QUALIFIED'}")
            if missing_requirements:
                self.logger.info(f"Missing requirements: {', '.join(missing_requirements)}")
            
            return is_qualified, missing_requirements
            
        except Exception as e:
            self.logger.error(f"Error checking lead qualification for {lead_id}: {str(e)}")
            return False, ["Error checking qualification status"]
    
    def generate_contractor_notification_email(self, lead_id: str, contractor_email: str) -> Dict[str, str]:
        """
        ...
        try:
            ...
            if not is_qualified:
                self.logger.warning(f"Attempting to notify contractor for unqualified lead {lead_id}")
            ...
            # Generate email body
            body = self._create_contractor_email_body(lead_data, lead_summary, is_qualified)
            ...
            self.logger.info(f"Generated contractor notification email for lead {lead_id}")
            return email_data
            
        except Exception as e:
            self.logger.error(f"Error generating contractor notification email for {lead_id}: {str(e)}")
            raise
    
    def generate_customer_handoff_message(self, lead_id: str, contractor_info: Dict) -> Dict[str, str]:
        """
        ...
        try:
            ...
            estimated_contact_time = contractor_info.get('estimated_contact_time', '24 hours')
...
"""
            ...
            self.logger.error(f"Error generating customer handoff message for {lead_id}: {str(e)}")
            raise
    ...
    def format_lead_summary(self, lead_data: Dict) -> Dict[str, any]:
        ...
        try:
            ...
            summary['customer'] = {
                ...
                'preferred_contact_method': lead_data.get('preferred_contact_method', 'phone')
            }
            ...
            self.logger.info(f"Formatted lead summary for lead {lead_data.get('lead_id')}")
            return summary
    ...
    def log_notification(self, notification_data: Dict) -> str:
        ...
        try:
            ...
            self.logger.info(f"Logged notification {notification_id} for lead {notification_data.get('lead_id')}")
    ...
    def update_lead_status(self, lead_id: str, new_status: str = 'completed', contractor_id: str = None) -> bool:
        """
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"Updated lead {lead_id} status from '{old_status}' to '{new_status}'")
                return True
            else:
    ...
    def send_notification(self, notification_data: Dict, attachments: List[str] = None) -> bool:
        """
        ...
        try:
            ...
            self.logger.info(f"Successfully sent notification to {notification_data['recipient']}")
            return True
    ...
    def process_lead_handoff(self, lead_id: str, contractor_info: Dict) -> Dict[str, any]:
        """
    ...
    def _create_contractor_email_body(self, lead_data: Dict, lead_summary: Dict, is_qualified: bool) -> str:
        """Creates formatted contractor email body."""
        
        qualification_status = "✅ QUALIFIED LEAD" if is_qualified else "⚠️ NEEDS REVIEW"
        ...
        if photos:
            ...
            for i, photo in enumerate(photos, 1):
                ...
                if photo.get('analysis'):
                    body += f" (Analysis: {photo['analysis'].get('summary', 'Available')})"
                body += "\n"
