...
class ContractExtractor:
    ...
    def _load_extraction_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            'contract_number': re.compile(r'(?:contract\s+(?:number|no\.?|#)\s*[:\-]?\s*)([A-Z0-9\-\_]+)', re.IGNORECASE),
            'date': re.compile(r'(\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4}|\d{4}[\/\-\.]\d{1,2}[\/\-\.]\d{1,2})', re.IGNORECASE),
            'amount': re.compile(r'(?:\$|USD|EUR|GBP)\s?([\d,]+\.?\d*)', re.IGNORECASE),
            'party_names': re.compile(r'(?:party|client|contractor|vendor|supplier)\s*[:\-]?\s*([A-Za-z\s&\.,Inc]+)', re.IGNORECASE),
            'email': re.compile(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'),
            'phone': re.compile(r'(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}'),
            'address': re.compile(r'\d+\s+[A-Za-z\s,]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr)', re.IGNORECASE)
        }
    ...
    def extract_from_text_file(self, file_path: str, encoding: str = 'utf-8') -> Dict[str, Any]:
        """
    ...
    def extract_from_database(self, query: str, connection_params: Dict[str, str], db_type: str = 'postgresql') -> List[Dict[str, Any]]:
        """
    ...
    def extract_from_csv(self, file_path: str, mapping: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
        """
    ...
    def _extract_from_text(self, text_content: str, source: str) -> Dict[str, Any]:
        ...
        extracted_data = {
            'source': source,
            'extraction_date': datetime.now().isoformat(),
            'raw_text': text_content[:1000] if len(text_content) > 1000 else text_content  # Truncate for storage
        }
        ...
        if dates:
            ...
            extracted_data['contract_date'] = dates[0]  # Assume first date is contract date
        ...
        if parties:
            extracted_data['parties'] = [party.strip() for party in parties[:5]]  # Limit to 5 parties
        ...
        if phones:
            extracted_data['phone_numbers'] = list(set(phones[:3]))  # Unique phones, max 3
        
        addresses = self.patterns['address'].findall(text_content)
        if addresses:
            extracted_data['addresses'] = list(set(addresses[:3]))  # Unique addresses, max 3
        
        return extracted_data
    
    def _extract_from_postgresql(self, query: str, connection_params: Dict[str, str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                contract_data['source'] = f"postgresql:{connection_params.get('host', 'localhost')}"
    ...
    def _extract_from_sqlite(self, query: str, connection_params: Dict[str, str]) -> List[Dict[str, Any]]:
        ...
        for standard_field, possible_fields in field_mappings.items():
            for field in possible_fields:
                if field in raw_data and raw_data[field] is not None:
                    ...
                    if standard_field == 'contract_amount' and isinstance(value, (str, int, float)):
                        try:
    ...
    for source in sources:
        try:
            ...
            if source_type == 'pdf':
                ...
                logger.warning(f"Unsupported source type: {source_type} for source: {source}")
...
def _extract_from_config(config: Dict[str, Any], extractor: ContractExtractor) -> List[Dict[str, Any]]:
            ...
            try:
                if file_type == 'pdf':
                    ...
                    csv_results = extractor.extract_from_csv(
                        file_path, 
                        file_config.get('column_mapping')
                    )
                    results.extend(csv_results)
                    continue
                else:
