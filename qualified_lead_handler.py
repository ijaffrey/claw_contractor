...
class QualifiedLeadHandler:
    ...
    def log_processing_step(
        ...
        try:
            self.notification_logger.log_processing_step(
                lead_id=lead_id, step=step, status=status, details=details or {}
            )
        except Exception as e:
            # Don't fail the whole process for logging errors
            logger.warning(
                f"Failed to log processing step {step} for lead {lead_id}: {e}"
            )

    def send_contractor_notifications(
        self, lead_data: Dict[str, Any], lead_summary: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        for contractor in contractors:
            try:
                ...
                logger.info(
                    f"Successfully notified contractor {contractor['name']} about lead {lead_data['id']}"
                )
        ...
        failed_notifications = [
            r for r in notification_results if r["status"] == "error"
        ]
    ...
    def send_customer_handoff_message(
        ...
        try:
            ...
            logger.info(
                f"Successfully sent handoff message to customer for lead {lead_data['id']}"
            )
            return result
    ...
    def process_qualified_lead(self, lead_id: int) -> Dict[str, Any]:
        ...
        try:
            with self.database_transaction() as connection:
                ...
                self.log_processing_step(
                    ...
                    "success",
                    {
                        ...
                        "contractor_count": len(
                            lead_data.get("matched_contractors", [])
                        ),
                    },
                )
                ...
                processing_results["steps"]["qualification_check"] = {
                    "status": "success"
                }
                ...
                contractor_results = self.send_contractor_notifications(
                    lead_data, lead_summary
                )
                processing_results["steps"]["contractor_notifications"] = {
                    ...
                    "successful_notifications": len(
                        [r for r in contractor_results if r["status"] == "success"]
                    ),
                }
                self.log_processing_step(
                    lead_id,
                ...
                self.log_processing_step(
                    lead_id, "status_update", "success", {"new_status": "completed"}
                )
            ...
            self.log_processing_step(
                ...
                "workflow_completion",
                "error",
                {"error": str(e), "processing_results": processing_results},
            )
