...
def emergency_fallback_reviewer(
    code_content: str, context: Optional[str] = None
) -> dict:
    """
            ...
            try:
                ...
                try:
                    ...
                    if result.get("success", False):
                        ...
                        if (
                            "review" in user_input.lower()
                            or "enhance" in user_input.lower()
                        ):
                            fallback_input = input(
                                "Would you like to try the emergency fallback reviewer? (y/N): "
                            )
                            if fallback_input.lower() == "y":
                                fallback_result = emergency_fallback_reviewer(
                                    user_input
                                )
                                if fallback_result.get("success"):
                                    print(
                                        "✅ Emergency fallback completed successfully!"
                                    )
                                else:
                                    print(
                                        f"❌ Emergency fallback also failed: {fallback_result.get('error')}"
                                    )
                    ...
                    if "review" in user_input.lower():
                        fallback_input = input(
                            "Would you like to try the emergency fallback? (y/N): "
                        )
                        if fallback_input.lower() == "y":
                            ...
                            if result.get("success"):
                                ...
                                print(
                                    f"❌ Emergency fallback failed: {result.get('error')}"
                                )
