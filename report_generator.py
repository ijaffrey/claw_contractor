...
class ReportGenerator:
    ...
    def generate_report(
        ...
        try:
            ...
            if not filename:
                ...
                filename = (
                    f"{title.lower().replace(' ', '_')}_{timestamp}.{format.value}"
                )
            ...
            if format == ReportFormat.JSON:
                self._generate_json_report(
                    data, report_metadata, file_path, report_config
                )
    ...
    def _generate_json_report(
        ...
        report_data = {
            ...
            "summary": (
                self._generate_summary(data) if config["include_summary"] else {}
            ),
    ...
    def _generate_text_report(
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            ...
            if config["include_metadata"]:
                ...
                f.write("-" * 40 + "\n")
                f.write(
                    f"Generated: {metadata.generated_at.strftime(config['date_format'])}\n"
                )
                f.write(f"Total Records: {metadata.total_records}\n")
    ...
    def _build_html_report(
        self, data: Any, metadata: ReportMetadata, config: Dict[str, Any]
    ) -> str:
        ...
        if config["include_summary"]:
            summary = self._generate_summary(data)
            if summary:
                html_parts.extend(['<div class="summary">', "<h2>Summary</h2>"])
    ...
    def _prepare_data_for_pdf(
        self, data: Any, max_rows: int = 50
        ...
        try:
            ...
            combined_data = {
                ...
                "combined_records": sum(
                    self._count_records(data) for data in datasets.values()
                ),
            }
            ...
            return self.generate_report(
                data=combined_data, title=title, format=format, metadata=metadata
            )
            ...
            try:
                if PANDAS_AVAILABLE and isinstance(data, pd.DataFrame):
                    ...
                    if len(numeric_cols) > 0:
                        ...
                        chart_path = (
                            self.output_dir
                            / f"chart_{name.lower().replace(' ', '_')}.png"
                        )
