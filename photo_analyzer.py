...
def analyze_photo(
    image_data: bytes, trade_type: str, content_type: str = "image/jpeg"
) -> Optional[str]:
    """
...
def analyze_multiple_photos(photos: list, trade_type: str) -> Optional[str]:
    ...
    for i, photo in enumerate(photos[:5]):  # Limit to 5 photos max
        analysis = analyze_photo(
            photo["data"], trade_type, photo.get("content_type", "image/jpeg")
        )
        if analysis:
            if len(photos) > 1:
...
def format_lead_summary_with_photos(
    lead_data: Dict, photo_analysis: Optional[str]
) -> str:
    """
