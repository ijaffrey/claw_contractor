...
def get_next_question(
    questions: List[Question], answered_questions: Dict[str, Any]
) -> Optional[Question]:
    """
...
def get_required_questions(questions: List[Question]) -> List[Question]:
    ...
    logger.debug(
        f"Found {len(required)} required questions out of {len(questions)} total"
    )
    return required


def calculate_flow_completeness(
    questions: List[Question], answered_questions: Dict[str, Any]
) -> Dict[str, float]:
    """
    ...
    overall_completeness = (
        answered_count / total_questions if total_questions > 0 else 1.0
    )
    required_completeness = (
        required_answered / required_count if required_count > 0 else 1.0
    )
