...
class Lead(Base):
    ...
    notifications = relationship(
        "NotificationLog", back_populates="lead", cascade="all, delete-orphan"
    )
...
def _run_custom_migrations():
    ...
    try:
        ...
        notifications_without_status = (
            db.query(NotificationLog).filter(NotificationLog.status.is_(None)).all()
        )
    ...
    try:
        ...
        for notification_type in NotificationType:
            count = (
                ...
                .filter(NotificationLog.type == notification_type)
                .count()
            )
...
class LeadRepository:
    ...
    @staticmethod
    def get_leads(
        db: Session, skip: int = 0, limit: int = 100, status: LeadStatus = None
    ) -> List[Lead]:
...
class NotificationRepository:
    ...
    def get_notifications_by_lead(db: Session, lead_id: int) -> List[NotificationLog]:
        ...
        return (
            db.query(NotificationLog)
            .filter(NotificationLog.lead_id == lead_id)
...
try:
    ...
    logger.warning(
        "conversation_schema module not found - conversation tables will not be created"
    )
