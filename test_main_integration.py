...
class TestMainLoopIntegration:
    ...
    @pytest.mark.asyncio
    async def test_main_loop_processes_emails_and_detects_qualified_leads(self, temp_db, mock_email_data, qualified_lead_data):
        ...
        with patch('main.get_db_session') as mock_get_session:
            ...
            with patch('main.EmailProcessor') as mock_email_processor, \
                ...
                qualified_calls = [call for call in mock_qlh.handle_qualified_lead.call_args_list]
                assert len(qualified_calls) > 0

    @pytest.mark.asyncio
    async def test_qualified_leads_trigger_handoff_without_breaking_loop(self, temp_db, mock_email_data):
    ...
    @pytest.mark.asyncio
    async def test_non_qualified_leads_continue_normal_processing(self, temp_db, mock_email_data):
        ...
        with patch('main.get_db_session') as mock_get_session:
            ...
            with patch('main.EmailProcessor') as mock_email_processor, \
                ...
                mock_qualifier = Mock()
                mock_lead_qualifier.return_value = mock_qualifier
                mock_qualifier.analyze_email.return_value = {'budget': 500}  # Low budget
    ...
    @pytest.mark.asyncio
    async def test_handoff_error_handling_doesnt_crash_main_loop(self, temp_db, mock_email_data):
        ...
        with patch('main.get_db_session') as mock_get_session:
            ...
            with patch('main.EmailProcessor') as mock_email_processor, \
                ...
                mock_qlh = AsyncMock()
                mock_handler.return_value = mock_qlh
                mock_qlh.handle_qualified_lead.side_effect = Exception("Handoff service unavailable")
                ...
                assert mock_processor.process_email.call_count >= 6  # 3 emails × 2 iterations

    @pytest.mark.asyncio
    async def test_database_state_consistency_after_qualified_lead_processing(self, temp_db, mock_email_data):
        ...
        client = Client(email='ceo@fintechstartup.com', name='John Smith', company='FinTech Startup Inc.')
        session.add(user)
        session.add(client)
        session.commit()
        ...
        with patch('main.get_db_session') as mock_get_session:
            ...
            with patch('main.EmailProcessor') as mock_email_processor, \
                ...
                mock_processor = AsyncMock()
                mock_email_processor.return_value = mock_processor
                mock_processor.fetch_new_emails.return_value = [mock_email_data[0]]  # One qualified email
                ...
                mock_cm = Mock()
                mock_conv_manager.return_value = mock_cm
                conversation = Conversation(client_id=client.id, user_id=user.id, status='active')
                ...
                mock_qlh.handle_qualified_lead.side_effect = handle_qualified_lead_side_effect
                ...
                assert qualified_leads[0].conversation.client.email == 'ceo@fintechstartup.com'
        ...
        with patch('main.get_db_session') as mock_get_session:
            ...
            with patch('main.EmailProcessor') as mock_email_processor, \
                ...
                def analyze_email_side_effect(email_data):
                    if 'enterprise' in email_data['from_email'] or 'fintech' in email_data['from_email']:
    ...
    async def test_concurrent_processing_safety(self, temp_db, mock_email_data):
        ...
        with patch('main.get_db_session') as mock_get_session:
            ...
            session1 = Session()
            session2 = Session()
            mock_get_session.side_effect = [session1, session2] * 10  # Alternate sessions
    ...
    async def test_email_processing_rollback_on_handoff_failure(self, temp_db):
        ...
        with patch('main.get_db_session') as mock_get_session:
            ...
            with patch('main.EmailProcessor') as mock_email_processor, \
                ...
                mock_qualifier = Mock()
                mock_lead_qualifier.return_value = mock_qualifier
                mock_qualifier.analyze_email.return_value = {'budget': 1000000, 'critical': True}
                mock_qualifier.is_qualified_lead.return_value = True
                ...
                mock_qlh = AsyncMock()
                mock_handler.return_value = mock_qlh
                mock_qlh.handle_qualified_lead.side_effect = Exception("Critical handoff system failure")
