...
class QualifiedLeadDetector:
    ...
    def analyze_lead_qualification(
        self, conversation_state: Dict[str, Any], lead_data: Dict[str, Any]
    ) -> QualificationResult:
        """
        ...
        try:
            ...
            recommendations = self._generate_recommendations(
                score, conversation_state, lead_data
            )

            # Create contractor notes
            contractor_notes = self._generate_contractor_notes(
    ...
    def validate_lead_completeness(
    ...
    def _calculate_qualification_score(
        ...
        for criteria in QualificationCriteria:
            ...
            criteria_score = self._score_criteria(
                criteria, conversation_state, lead_data
            )
    ...
    def _score_criteria(
        ...
        try:
            if criteria == QualificationCriteria.CONTACT_INFO:
                ...
                return (
                    1.0
                    if self._has_urgency_assessment(conversation_state, lead_data)
                    else 0.0
                )
                ...
                return (
                    1.0
                    if self._has_budget_discussion(conversation_state, lead_data)
                    else 0.0
                )
                ...
                return (
                    1.0
                    if self._has_availability_info(conversation_state, lead_data)
                    else 0.0
                )
                ...
                return (
                    1.0
                    if self._has_diagnosis_complete(conversation_state, lead_data)
                    else 0.0
                )
    ...
    def _has_budget_discussion(
        ...
        budget_discussed = conversation_state.get("topics_discussed", {}).get(
            "budget", False
        )
    ...
    def _score_problem_description(
        ...
        detail_count = sum(
            1 for keyword in detail_keywords if keyword.lower() in description.lower()
        )
    ...
    def _is_ready_for_handoff(
        ...
        all_collected, _ = self.check_required_data_collected(
            conversation_state, lead_data
        )
    ...
    def _get_next_steps(
        ...
        critical_missing = [
            criteria
            for criteria in score.missing_criteria
            if criteria
            in [
    ...
    def _estimate_completion_time(
        self, score: QualificationScore, conversation_state: Dict[str, Any]
    ) -> Optional[datetime]:
        ...
        if conversation_start:
            start_time = (
                datetime.fromisoformat(conversation_start)
                if isinstance(conversation_start, str)
            ...
            if score.completion_percentage > 0:
                estimated_total_time = elapsed_time * (
                    100 / score.completion_percentage
                )
