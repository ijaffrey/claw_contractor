...
class TestFinalHandoffIntegration:
    ...
    async def test_complete_handoff_workflow_success(
        ...
        # Setup test data
        lead, contractor = self.setup_test_data(test_db, sample_lead_data, sample_contractor_data)
    ...
    async def test_handoff_with_contractor_notification_failure(
        ...
        failed_notification = test_db.query(Notification).filter_by(
            lead_id=lead.id, 
    ...
    async def test_notification_logging_throughout_process(
        ...
        notifications = test_db.query(Notification).filter_by(lead_id=lead.id).order_by(
    ...
    async def test_concurrent_handoff_requests_for_same_lead(
        ...
        assert "already in progress" in str(failed_results[0]) or "already completed" in str(failed_results[0])
    ...
    async def test_handoff_rollback_on_partial_failure(
        ...
        contractor_notification = next(n for n in notifications if n.recipient_type == "contractor")
        customer_notification = next(n for n in notifications if n.recipient_type == "customer")
        
        assert contractor_notification.status == "sent"
        assert customer_notification.status == "failed"
        assert customer_notification.error_message == "Customer notification service unavailable"