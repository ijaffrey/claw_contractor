...
class TestDataCleaner:
    ...
    def clean_test_database(self, db_path: str = "test_leads.db") -> Dict[str, int]:
        ...
        # Clean old leads
        cursor.execute("DELETE FROM leads WHERE created_at < ?", (cutoff_iso,))
        leads_removed = cursor.rowcount

        # Clean old conversations
        cursor.execute(
            "DELETE FROM conversations WHERE last_activity < ?", (cutoff_iso,)
        )
        ...
        self.logger.info(
            f"Cleaned {leads_removed} old leads, {conversations_removed} old conversations"
        )
