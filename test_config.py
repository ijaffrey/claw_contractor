    ...
    def create_mock_openai_client(response_type: str = "email_generation_success"):
        ...
        if response_type in MOCK_OPENAI_RESPONSES:
            if "error" in MOCK_OPENAI_RESPONSES[response_type]:
                ...
                mock_response.model_dump.return_value = MOCK_OPENAI_RESPONSES[
                    response_type
                ]
                mock_response.choices[0].message.content = MOCK_OPENAI_RESPONSES[
                    response_type
    ...
    def create_mock_gmail_service(response_type: str = "send_success"):
        ...
        if response_type == "send_success":
            mock_service.users().messages().send().execute.return_value = (
                MOCK_GMAIL_RESPONSES["send_success"]
            )
...
class TestFixtures:
    ...
    @classmethod
    def create_email_template_with_custom_data(
        cls, base_template: str, **kwargs
    ) -> Dict[str, str]:
