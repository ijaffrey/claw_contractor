...
def process_handoff(lead_id):
    ...
    try:
        ...
        # Process handoff
        logger.info(f"Processing handoff for lead {lead_id} by user {request.current_user.id}")
        ...
        if not result.success:
            logger.error(f"Handoff processing failed for lead {lead_id}: {result.error}")
            return format_error_response(
                result.error,
        ...
        logger.warning(f"Validation error processing handoff for lead {lead_id}: {str(e)}")
        return format_error_response(
        ...
        logger.warning(f"Permission denied processing handoff for lead {lead_id}: {str(e)}")
        return format_error_response(
        ...
        logger.error(f"Unexpected error processing handoff for lead {lead_id}: {str(e)}")
        logger.error(traceback.format_exc())
        return format_error_response(
            'An unexpected error occurred while processing handoff',
            500
        ), 500
...
def get_handoff_status(lead_id):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
        return format_error_response(
            'An unexpected error occurred while retrieving handoff status',
            500
        ), 500
...
def retry_handoff(lead_id):
    ...
    try:
        ...
        if lead.handoff_status != 'failed':
            ...
            return format_error_response(
                'Lead does not have a failed handoff to retry',
                400
            ), 400
        
        # Retry handoff
        logger.info(f"Retrying handoff for lead {lead_id} by user {request.current_user.id}")
        ...
        return format_success_response(
            data=response_data,
            message='Handoff retry processed successfully'
        ), 200
        ...
        logger.error(traceback.format_exc())
        return format_error_response(
            'An unexpected error occurred while retrying handoff',
            500
        ), 500
