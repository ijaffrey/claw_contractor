...
class LeadHandoffService:
    ...
    def _determine_urgency_level(self, lead: Lead, responses: List) -> str:
        ...
        try:
            urgency_keywords = {
                'high': ['urgent', 'emergency', 'asap', 'immediately', 'leak', 'broken'],
    ...
    async def _generate_summary_text(
        ...
        try:
            ...
            project_type = lead.metadata.get('project_type', 'General').replace('_', ' ').title()
            summary_parts.append(f"Project Type: {project_type}")
            
            if project_details.get('scope_defined'):
                summary_parts.append(f"Scope: {project_details.get('scope_description', 'Defined')}")
    ...
    async def coordinate_contractor_notifications(
        ...
        target_contractors: Optional[List[str]] = None
    ) -> List[str]:
        """
        ...
        try:
            ...
            for contractor_id in target_contractors:
                try:
                    contractor = await self.contractor_service.get_contractor(contractor_id)
                    if not contractor:
                    ...
                    # Send notification
                    success = await self.notification_service.send_contractor_notification(
                        contractor_id=contractor_id,
                        content=notification_content,
                        priority='high' if lead_summary.urgency_level == 'high' else 'normal'
                    )
                    
                    if success:
                        notified_contractors.append(contractor_id)
                        logger.info(f"Notified contractor {contractor_id} about lead {lead_summary.lead_id}")
                    else:
    ...
    def _create_contractor_notification(
        ...
        return {
            ...
            'project_type': lead_summary.project_details.get('project_type', 'General').replace('_', ' ').title(),
            'location': lead_summary.customer_info.get('address', {}).get('city', 'Unknown'),
    ...
    async def orchestrate_handoff_workflow(self, lead_id: str) -> HandoffResult:
        ...
        try:
            ...
            notified_contractors = await self.coordinate_contractor_notifications(lead_summary)
            ...
            await self._send_customer_handoff_confirmation(lead_id, len(notified_contractors))
    ...
    async def get_handoff_analytics(self, date_range: Tuple[datetime, datetime]) -> Dict:
        ...
        try:
            ...
            successful_handoffs = [r for r in handoff_records if r.get('handoff_completed_at')]
            ...
            return {
                ...
                'total_estimated_value': sum(
                    r.get('estimated_value', 0) for r in handoff_records
