...
class ConversationManager:
    ...
    def load_conversation_history(self, lead_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error loading conversation history for lead_id {lead_id}: {str(e)}")
    ...
    def get_conversation_context(self, lead_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            context = {
                ...
                'conversation_status': conversation_record.get('status', 'active') if conversation_record else 'new',
                'thread_id': conversation_record.get('thread_id') if conversation_record else None,
                'last_activity': conversation_history[-1]['timestamp'] if conversation_history else None,
                'message_count': len(conversation_history),
                'qualification_progress': self._calculate_qualification_progress(conversation_history),
                'context_summary': self._generate_context_summary(conversation_history, lead_info)
            }
            
            logger.info(f"Generated context for lead_id {lead_id}: step={current_step}, status={context['conversation_status']}")
            return context
            
        except Exception as e:
            logger.error(f"Error getting conversation context for lead_id {lead_id}: {str(e)}")
    ...
    def update_thread_id(self, lead_id: str, thread_id: str) -> bool:
        ...
        """
        try:
            logger.info(f"Updating thread_id for lead_id: {lead_id}, thread_id: {thread_id}")
            ...
            cursor = self.database.execute_query(update_query, (thread_id, conversation_id))
            
            if cursor.rowcount == 0:
                logger.warning(f"No conversation found to update for lead_id: {lead_id}")
                return False
    ...
    def get_next_qualification_step(self, lead_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            qualification_state = self._analyze_qualification_state(conversation_history)
            ...
            logger.info(f"Next qualification step for lead_id {lead_id}: {next_step['step_name']}")
            return next_step
            
        except Exception as e:
            logger.error(f"Error determining next qualification step for lead_id {lead_id}: {str(e)}")
            raise Exception(f"Failed to determine next qualification step: {str(e)}")
    
    def add_message(self, lead_id: str, content: str, sender_type: str, 
                   message_type: str = 'email', metadata: Optional[Dict] = None) -> str:
        """
        ...
        """
        try:
            logger.info(f"Adding message for lead_id: {lead_id}, sender_type: {sender_type}")
    ...
    def update_conversation_status(self, lead_id: str, status: str) -> bool:
        ...
        """
        try:
            logger.info(f"Updating conversation status for lead_id: {lead_id} to: {status}")
            ...
            self.database.commit()
            logger.info(f"Successfully updated conversation status for lead_id: {lead_id}")
            return True
            
        except Exception as e:
            logger.error(f"Error updating conversation status for lead_id {lead_id}: {str(e)}")
            self.database.rollback()
            return False
    ...
    def _get_conversation_record(self, lead_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting conversation record for lead_id {lead_id}: {str(e)}")
            return None
    ...
    def _get_or_create_conversation(self, lead_id: str) -> str:
        ...
        try:
            ...
            logger.info(f"Created new conversation with ID: {conversation_id} for lead_id: {lead_id}")
            return str(conversation_id)
            
        except Exception as e:
            logger.error(f"Error getting/creating conversation for lead_id {lead_id}: {str(e)}")
            raise Exception(f"Failed to get or create conversation: {str(e)}")
    
    def _determine_current_qualification_step(self, conversation_history: List[Dict], 
                                            conversation_record: Optional[Dict]) -> str:
    ...
    def _analyze_qualification_state(self, conversation_history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.qualification_manager.analyze_qualification_state(conversation_history)
            
        except Exception as e:
            logger.error(f"Error analyzing qualification state: {str(e)}")
            return {'step': 'initial_contact', 'completed_steps': [], 'gathered_info': {}}
    
    def _calculate_qualification_progress(self, conversation_history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_steps = self.qualification_manager.count_completed_steps(conversation_history)
    ...
    def _generate_context_summary(self, conversation_history: List[Dict], 
    ...
    def _update_conversation_after_message(self, conversation_id: str, sender_type: str):
