...
class EmailFormatter:
    ...
    def _sanitize_lead_data(self, lead_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in lead_data.items():
            if isinstance(value, str):
                ...
                sanitized[key] = [
                    (
                        self.sanitize_customer_input(item)
    ...
    def _format_html_lead_summary(self, lead_data: Dict[str, Any]) -> str:
        ...
        if any(key in lead_data for key in ["customer_name", "email", "phone"]):
            ...
            if "customer_name" in lead_data and lead_data["customer_name"]:
                html_parts.append(
                    f'<tr><td><strong>Name:</strong></td><td>{lead_data["customer_name"]}</td></tr>'
                )

            if "email" in lead_data and lead_data["email"]:
                html_parts.append(
                    f'<tr><td><strong>Email:</strong></td><td><a href="mailto:{lead_data["email"]}">{lead_data["email"]}</a></td></tr>'
                )

            if "phone" in lead_data and lead_data["phone"]:
                formatted_phone = self._format_phone_number(lead_data["phone"])
                html_parts.append(
                    f'<tr><td><strong>Phone:</strong></td><td><a href="tel:{lead_data["phone"]}">{formatted_phone}</a></td></tr>'
                )
        ...
        if any(
            key in lead_data
            for key in ["service_type", "description", "address", "preferred_date"]
        ):
            ...
            if "service_type" in lead_data and lead_data["service_type"]:
                html_parts.append(
                    f'<tr><td><strong>Service Type:</strong></td><td>{lead_data["service_type"]}</td></tr>'
                )

            if "address" in lead_data and lead_data["address"]:
                address_link = f"https://maps.google.com/maps?q={lead_data['address'].replace(' ', '+')}"
                html_parts.append(
                    f'<tr><td><strong>Address:</strong></td><td><a href="{address_link}" target="_blank">{lead_data["address"]}</a></td></tr>'
                )

            if "preferred_date" in lead_data and lead_data["preferred_date"]:
                html_parts.append(
                    f'<tr><td><strong>Preferred Date:</strong></td><td>{lead_data["preferred_date"]}</td></tr>'
                )
            ...
            if "description" in lead_data and lead_data["description"]:
                html_parts.append(
                    f'<h4>Description:</h4><div class="description">{lead_data["description"].replace(chr(10), "<br>")}</div>'
                )

        # Lead metadata
        if any(
            key in lead_data
            for key in ["lead_source", "lead_id", "created_at", "urgency"]
        ):
            ...
            if "lead_id" in lead_data and lead_data["lead_id"]:
                html_parts.append(
                    f'<tr><td><strong>Lead ID:</strong></td><td>{lead_data["lead_id"]}</td></tr>'
                )

            if "lead_source" in lead_data and lead_data["lead_source"]:
                html_parts.append(
                    f'<tr><td><strong>Source:</strong></td><td>{lead_data["lead_source"]}</td></tr>'
                )

            if "urgency" in lead_data and lead_data["urgency"]:
                urgency_class = f"urgency-{lead_data['urgency'].lower()}"
                html_parts.append(
                    f'<tr><td><strong>Urgency:</strong></td><td><span class="{urgency_class}">{lead_data["urgency"]}</span></td></tr>'
                )

            if "created_at" in lead_data and lead_data["created_at"]:
                formatted_date = self._format_datetime(lead_data["created_at"])
                html_parts.append(
                    f"<tr><td><strong>Created:</strong></td><td>{formatted_date}</td></tr>"
                )
    ...
    def _format_photos_html(self, photos: List[Dict[str, Any]]) -> str:
        ...
        for i, photo in enumerate(photos):
            if "url" in photo:
                ...
                html_parts.append(
                    f'<a href="{photo["url"]}" target="_blank">{photo_name}</a>'
                )
                if self._is_image_url(photo["url"]):
                    html_parts.append(
                        f'<br><img src="{photo["url"]}" alt="{photo_name}" style="max-width: 200px; max-height: 200px; margin-top: 5px;">'
                    )
                html_parts.append("</div>")
            elif "data" in photo:
                ...
                html_parts.append(
                    f'<div class="photo-item">📎 {photo_name} (attached)</div>'
                )
    ...
    def _format_base64_attachment(
        self, photo: Dict[str, Any], index: int
    ...
    def _get_notification_subject(
        self, notification_type: str, lead_data: Dict[str, Any]
    ) -> str:
        ...
        return subject_templates.get(
            notification_type, f"Contractor CRM Notification - {customer_name}"
        )
    ...
    def _strip_dangerous_html(self, text: str, allowed_tags: List[str]) -> str:
        ...
        text = re.sub(
            f"<(?!/?({allowed_pattern})\b)[^>]*>", "", text, flags=re.IGNORECASE
        )
