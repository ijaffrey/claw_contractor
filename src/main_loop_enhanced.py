...
class EnhancedMainLoop:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.qualification_check_interval = config.get(
            "qualification_check_interval", 1
        )
    ...
    async def start_conversation(
        ...
        try:
            ...
            response = await self._process_message(
                conversation_id, initial_message, is_initial=True
            )

            # Track conversation start
            await self.analytics_tracker.track_conversation_start(
                conversation_id, customer_info
            )
    ...
    async def process_message(
        ...
        try:
            if conversation_id not in self.active_conversations:
                logger.warning(
                    f"Conversation {conversation_id} not found, starting new conversation"
                )
            ...
            # Process the message
            response = await self._process_message(
                conversation_id, message, message_metadata
            )
    ...
    async def _process_message(
        ...
        try:
            ...
            await self.context_manager.add_message(
                conversation_id, "customer", message, metadata=message_metadata
            )

            # Generate response
            bot_response = await self.response_generator.generate_response(
                conversation_id, message, context, conversation_state["customer_info"]
            )
            ...
            response = {
                ...
                "metadata": {
                    ...
                    "qualification_score": conversation_state.get(
    ...
    async def _check_lead_qualification(
        self, conversation_id: str, customer_message: str, bot_response: Dict[str, Any]
    ):
        ...
        try:
            ...
            qualification_result = await self.lead_qualifier.qualify_lead(
                conversation_id, context, conversation_state["customer_info"]
            )

            # Update conversation state with qualification results
            conversation_state["qualification_score"] = qualification_result.get(
                "score", 0
            )
            conversation_state["qualification_factors"] = qualification_result.get(
                "factors", {}
            )
            ...
            if qualification_result.get(
                "qualified", False
            ) and not conversation_state.get("qualified", False):
            ...
            if len(conversation_state["interaction_scores"]) > 10:
                conversation_state["interaction_scores"] = conversation_state[
                    "interaction_scores"
                ][-10:]

        except Exception as e:
            logger.error(
                f"Error in lead qualification check for {conversation_id}: {str(e)}"
            )
            # Don't fail the entire interaction if qualification fails
            conversation_state["qualification_error"] = str(e)

    async def _should_trigger_automatic_handoff(
        self, qualification_result: Dict[str, Any]
    ) -> bool:
        ...
        ):  # Customer explicitly asked for sales
            return True
    ...
    async def _trigger_automatic_handoff(
        self, conversation_id: str, qualification_result: Dict[str, Any]
    ):
        """Trigger automatic handoff for qualified leads."""
        try:
            logger.info(
                f"Triggering automatic handoff for conversation {conversation_id}"
            )
            ...
            handoff_result = await self.handoff_orchestrator.initiate_handoff(
                conversation_id, handoff_data
            )

            # Update conversation state with handoff result
            conversation_state["handoff_result"] = handoff_result
            conversation_state["handoff_status"] = handoff_result.get(
                "status", "failed"
            )

            # Track handoff event
            await self.analytics_tracker.track_handoff_initiated(
                conversation_id, handoff_result
            )

            logger.info(
                f"Automatic handoff initiated for {conversation_id}: {handoff_result.get('status', 'unknown')}"
            )

        except Exception as e:
            logger.error(
                f"Error triggering automatic handoff for {conversation_id}: {str(e)}"
            )
            self.handoff_in_progress[conversation_id] = False
    ...
    def _calculate_handoff_priority(self, qualification_result: Dict[str, Any]) -> str:
        ...
        if (
            score >= 90
            or factors.get("explicit_sales_request", False)
            or factors.get("high_value_opportunity", False)
        ):
            return "high"
        ...
        if (
            score >= 70
            or factors.get("ready_to_buy", False)
            or factors.get("timeline_urgent", False)
        ):
            return "medium"
    ...
    def _should_offer_handoff(self, conversation_state: Dict[str, Any]) -> bool:
        ...
        if conversation_state.get("qualified", False) and not conversation_state.get(
            "handoff_attempted", False
        ):
            return True
    ...
    async def request_handoff(
        self, conversation_id: str, handoff_reason: str = "customer_request"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Processing handoff request for conversation {conversation_id}"
            )
            ...
            handoff_data = {
                ...
                "priority": (
                    "high" if handoff_reason == "customer_request" else "normal"
                ),
                "requested_at": datetime.now(timezone.utc).isoformat(),
            }
            ...
            logger.error(
                f"Error processing handoff request for {conversation_id}: {str(e)}"
            )
            self.handoff_in_progress[conversation_id] = False
            return {
                "success": False,
                "error": "Internal error processing handoff request",
            }

    async def _handle_handoff_in_progress(
        ...
        try:
            ...
            if handoff_id:
                ...
                status_update = await self.handoff_orchestrator.get_handoff_status(
                    handoff_id
                )
            ...
            await self.context_manager.add_message(
                conversation_id, "customer", message, metadata={"during_handoff": True}
            )
            ...
            return {
                ...
                "qualified": conversation_state.get("qualified", False),
                "metadata": {"handoff_in_progress": True, "handoff_id": handoff_id},
            }

        except Exception as e:
            logger.error(
                f"Error handling handoff in progress for {conversation_id}: {str(e)}"
            )
    ...
    async def _handle_error(
        self, conversation_id: str, error_type: str, error: Exception
    ) -> Dict[str, Any]:
        """Handle errors with retry logic and graceful degradation."""
        self.error_counts[conversation_id] = (
            self.error_counts.get(conversation_id, 0) + 1
        )

        # Track error
        await self.analytics_tracker.track_error(
            conversation_id, error_type, str(error)
        )

        # If too many errors, end conversation gracefully
        if self.error_counts[conversation_id] >= self.error_retry_limit:
            logger.error(
                f"Too many errors for conversation {conversation_id}, ending gracefully"
            )
    ...
    async def end_conversation(
        self, conversation_id: str, reason: str = "customer_ended"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.analytics_tracker.track_conversation_end(
                conversation_id, reason, conversation_state
            )
            ...
            if self.handoff_in_progress.get(conversation_id, False):
                ...
                if handoff_id:
                    try:
                        await self.handoff_orchestrator.cancel_handoff(
                            handoff_id, reason
                        )
                    except Exception as e:
        ...
        try:
            ...
            if self.handoff_in_progress.get(conversation_id, False):
                ...
                if handoff_id:
                    try:
                        status_update = (
                            await self.handoff_orchestrator.get_handoff_status(
                                handoff_id
                            )
                        )
            ...
            return {
                ...
                "handoff_in_progress": self.handoff_in_progress.get(
                    conversation_id, False
                ),
                "handoff_status": handoff_status,
                "last_message_at": (
                    conversation_state.get("last_message_at", "").isoformat()
                    if conversation_state.get("last_message_at")
                    else None
                ),
            ...
            logger.error(
                f"Error getting conversation status for {conversation_id}: {str(e)}"
            )
    ...
    async def cleanup_inactive_conversations(self, max_age_hours: int = 24):
        ...
        try:
            ...
            logger.info(
                f"Cleaned up {len(conversations_to_end)} inactive conversations"
            )
