...
class HandoffWorkflow:
    ...
    def _get_and_validate_lead(self, lead_id: str, result: HandoffResult) -> Optional[Lead]:
        ...
        try:
            ...
            self.logger.debug(f"Lead {lead_id} retrieved successfully, status: {lead.status}")
            return lead
    ...
    def _send_contractor_notification(self, lead: Lead, result: HandoffResult) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Contractor notification sent successfully for lead {lead.id}")
            return True
    ...
    def _send_customer_handoff_message(self, lead: Lead, result: HandoffResult) -> bool:
        ...
        try:
            ...
            message_data = {
                ...
                'service_type': lead.service_type,
                'estimated_response_time': contractor_info.get('response_time', '24 hours')
            }
            ...
            if not success:
                error_msg = f"Failed to send customer handoff message for lead {lead.id}"
                self.logger.error(error_msg)
                result.error_message = error_msg
                return False
            
            self.logger.info(f"Customer handoff message sent successfully for lead {lead.id}")
            return True
    ...
    def _log_notifications(self, lead: Lead, result: HandoffResult) -> bool:
        ...
        try:
            ...
            success = self.notification_service.log_workflow_notifications(notification_log_data)
    ...
    def _get_contractor_info(self, contractor_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error retrieving contractor info for {contractor_id}: {str(e)}")
            return None
    ...
    def _rollback_changes(self, lead_id: str, result: HandoffResult) -> None:
        ...
        try:
            ...
            if result.status_updated and result.previous_status:
                ...
                rollback_success = self.lead_service.update_lead_status(lead_id, rollback_data)
                if rollback_success:
