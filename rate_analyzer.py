...
class RateAnalyzer:
    ...
    def _validate_rate_data(self, data: List[RateData]) -> List[RateData]:
        ...
        for rate_record in data:
            if rate_record.rate <= 0:
                self.logger.warning(
                    f"Invalid rate {rate_record.rate} for contractor {rate_record.contractor_id}"
                )
                continue
    ...
    def analyze_rates(
        ...
        if currency_filter:
            validated_data = [
                d for d in validated_data if d.currency == currency_filter
            ]
            ...
            outlier_data.append(
                {
                    ...
                    "z_score": abs(validated_data[idx].rate - statistics.mean(rates))
        ...
        if not include_outliers and outlier_indices:
            filtered_rates = [
                rate for i, rate in enumerate(rates) if i not in outlier_indices
            ]
            if filtered_rates:
    ...
    def analyze_trends(
        self, data: Optional[List[RateData]] = None, time_period_months: int = 12
    ) -> TrendAnalysis:
        ...
        trend_strength = (
            min(1.0, max(0.0, abs(slope) / max_rate_change))
            if max_rate_change > 0
            else 0
        )
        if slope < 0:
    ...
    def comparative_analysis(
        self, data: Optional[List[RateData]] = None, top_n: int = 5
    ) -> ComparativeAnalysis:
            ...
            for group_name, rates in groups.items():
                if rates:
                    stats[group_name] = {
                        ...
                        "count": len(rates),
                        "std_dev": round(
                            statistics.stdev(rates) if len(rates) > 1 else 0, 2
                        ),
                        "min": min(rates),
                        "max": max(rates),
                    }
        ...
        for contractor_id, records in by_contractor.items():
            ...
            if rates:
                contractor_stats[contractor_id] = {
                    ...
                    "revenue": sum(
                        r["rate"] * r["hours"] for r in records if r["hours"] > 0
                    ),
        ...
        sorted_contractors = sorted(
            contractor_stats.values(), key=lambda x: x["avg_rate"], reverse=True
        )

        top_performers = sorted_contractors[:top_n]
        bottom_performers = (
            sorted_contractors[-top_n:] if len(sorted_contractors) > top_n else []
        )
...
def analyze_rates(
    rate_data: List[Dict[str, Any]], analysis_type: str = "basic", **kwargs
) -> Dict[str, Any]:
        ...
        if analysis_type == "basic":
            ...
            for field in required_fields:
                if field not in kwargs:
                    raise ValueError(
                        f"Missing required field for recommendations: {field}"
                    )
...
def calculate_rate_statistics(rates: List[float]) -> Dict[str, float]:
    ...
    return {
        ...
        "median": round(statistics.median(rates), 2),
        "mode": (
            round(statistics.mode(rates), 2) if len(set(rates)) < len(rates) else None
        ),
...
def detect_rate_anomalies(
    rates: List[float], method: str = "zscore", threshold: float = 2.0
) -> List[int]:
