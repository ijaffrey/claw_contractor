...
class TestEmailSender:
    ...
    @patch("smtplib.SMTP")
    def test_send_contractor_notification_success(
        self, mock_smtp_class, email_sender, contractor_data
    ):
        ...
        mock_smtp_instance.login.assert_called_once_with(
            email_sender.config.username, email_sender.config.password
        )
        ...
        assert (
            ...
            == f"New Project Assignment: {contractor_data['project_name']}"
        )
        assert contractor_data["contractor_name"] in sent_message.get_content()

    @patch("smtplib.SMTP")
    def test_send_contractor_notification_with_attachments(
        self, mock_smtp_class, email_sender, contractor_data
    ):
        ...
        # Execute
        result = email_sender.send_contractor_notification(
            contractor_data, attachments=attachments
        )
    ...
    @patch("smtplib.SMTP")
    def test_send_contractor_notification_smtp_error(
        self, mock_smtp_class, email_sender, contractor_data
    ):
    ...
    @patch("smtplib.SMTP")
    def test_send_customer_handoff_success(
        self, mock_smtp_class, email_sender, customer_handoff_data
    ):
        ...
        assert (
            sent_message["Subject"]
            == f"Project Handoff: {customer_handoff_data['project_name']}"
        )
        assert customer_handoff_data["contractor_name"] in sent_message.get_content()

    @patch("smtplib.SMTP")
    def test_send_customer_handoff_with_cc(
        self, mock_smtp_class, email_sender, customer_handoff_data
    ):
        ...
        # Execute
        result = email_sender.send_customer_handoff(
            customer_handoff_data, cc=cc_recipients
        )
    ...
    def test_format_email_html_contractor_template(self, email_sender, contractor_data):
        ...
        # Execute
        html_content = email_sender.format_email_html(
            "contractor_notification", contractor_data
        )
    ...
    def test_format_email_html_customer_handoff_template(
        self, email_sender, customer_handoff_data
    ):
        """Test HTML email formatting for customer handoff."""
        # Execute
        html_content = email_sender.format_email_html(
            "customer_handoff", customer_handoff_data
        )
    ...
    @patch("smtplib.SMTP")
    def test_handle_email_errors_smtp_exception(
        self, mock_smtp_class, email_sender, contractor_data
    ):
    ...
    @patch("smtplib.SMTP")
    def test_handle_email_errors_data_error(
        self, mock_smtp_class, email_sender, contractor_data
    ):
        ...
        mock_smtp_class.return_value = mock_smtp_instance
        mock_smtp_instance.__enter__.return_value = mock_smtp_instance
        mock_smtp_instance.send_message.side_effect = smtplib.SMTPDataError(
    ...
    @patch("smtplib.SMTP")
    def test_handle_email_errors_server_disconnect(
        self, mock_smtp_class, email_sender, contractor_data
    ):
...
class TestEmailSenderIntegration:
    ...
    @patch("smtplib.SMTP")
    def test_integration_full_email_workflow(
        self, mock_smtp_class, mock_email_server, email_config
    ):
    ...
    @patch("smtplib.SMTP")
    def test_integration_multiple_recipients(
        self, mock_smtp_class, mock_email_server, email_config
    ):
    ...
    def test_integration_error_recovery(self, mock_smtp_class, email_config):
        ...
        failing_server.send_message.side_effect = smtplib.SMTPException(
            "Temporary failure"
        )
    ...
    def test_integration_email_template_rendering(self, email_config):
        ...
        # Execute
        html_content = email_sender.format_email_html(
            "contractor_notification", template_data
        )
        ...
        assert all(
            str(value) in html_content
            for key, value in template_data.items()
            if key != "contractor_email"
        )  # Email might not be in body
