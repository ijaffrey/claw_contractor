...
def mock_email_service():
    ...
    service.send_template_email = AsyncMock(
        return_value={"message_id": "msg_456", "status": "sent"}
    )
    return service
...
class TestContractorNotificationEmail:
    ...
    @pytest.mark.asyncio
    async def test_generate_contractor_notification_email_content(
        self, workflow, qualified_lead, sample_contractors
    ):
        """Test generation of contractor notification email content."""
        contractor = sample_contractors[0]

        email_content = workflow._generate_contractor_email_content(
            qualified_lead, contractor
        )

        assert (
            email_content["subject"]
            == f"New Qualified Lead: {qualified_lead.project_type.value}"
        )
    ...
    @pytest.mark.asyncio
    async def test_send_contractor_notification_email_success(
        self, workflow, qualified_lead, sample_contractors, mock_email_service
    ):
        """Test successful sending of contractor notification email."""
        contractor = sample_contractors[0]

        result = await workflow._send_contractor_notification(
            qualified_lead, contractor
        )
    ...
    @pytest.mark.asyncio
    async def test_send_contractor_notification_email_failure(
        self, workflow, qualified_lead, sample_contractors, mock_email_service
    ):
        """Test handling of contractor notification email failure."""
        contractor = sample_contractors[0]
        mock_email_service.send_template_email.side_effect = NotificationError(
    ...
    @pytest.mark.asyncio
    async def test_send_multiple_contractor_notifications(
        self, workflow, qualified_lead, sample_contractors, mock_email_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_contractor_email_personalization(
        self, workflow, qualified_lead, sample_contractors
    ):
...
class TestLeadSummaryFormatting:
    ...
    def test_format_lead_summary_complete_data(self, workflow, qualified_lead):
        ...
        assert (
            f"Budget: ${qualified_lead.budget_min:,.2f} - ${qualified_lead.budget_max:,.2f}"
            in summary
        )
    ...
    def test_format_lead_summary_html_output(self, workflow, qualified_lead):
        ...
        assert (
            f"<p><strong>Customer:</strong> {qualified_lead.customer_name}</p>"
            in summary
        )
...
class TestCustomerHandoffMessage:
    ...
    def test_generate_customer_handoff_message_basic(
        ...
        message = workflow._generate_customer_handoff_message(
            qualified_lead, matched_contractors
        )
    ...
    def test_generate_customer_handoff_message_multiple_contractors(
        self, workflow, qualified_lead, sample_contractors
    ):
        """Test generation of customer handoff message with multiple contractors."""
        message = workflow._generate_customer_handoff_message(
            qualified_lead, sample_contractors
        )

        # Should mention multiple contractors
        assert "contractors" in message.lower()
        assert len(
            [name for name in [c.name for c in sample_contractors] if name in message]
        ) == len(sample_contractors)

    def test_generate_customer_handoff_message_project_details(
        self, workflow, qualified_lead, sample_contractors
    ):
    ...
    def test_generate_customer_handoff_message_next_steps(
        self, workflow, qualified_lead, sample_contractors
    ):
    ...
    def test_generate_customer_handoff_message_no_contractors(
        self, workflow, qualified_lead
    ):
        ...
        assert (
            "no contractors" in message.lower() or "unable to match" in message.lower()
        )
        assert "expand our search" in message.lower()

    @pytest.mark.asyncio
    async def test_send_customer_handoff_email(
        self, workflow, qualified_lead, sample_contractors, mock_email_service
    ):
        ...
        result = await workflow._send_customer_handoff_notification(
            qualified_lead, sample_contractors
        )
...
class TestLeadStatusUpdates:
    ...
    @pytest.mark.asyncio
    async def test_update_lead_status_to_qualified(
        self, workflow, sample_lead, mock_db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_lead_status_to_handed_off(
        self, workflow, qualified_lead, mock_db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_lead_status_to_unqualified(
        self, workflow, sample_lead, mock_db_session
    ):
        """Test updating lead status to unqualified."""
        await workflow._update_lead_status(
            sample_lead, LeadStatus.UNQUALIFIED, "Low qualification score"
        )

        assert sample_lead.status == LeadStatus.UNQUALIFIED
        assert "Low qualification score" in sample_lead.metadata.get(
            "unqualified_reason", ""
        )

    @pytest.mark.asyncio
    async def test_update_lead_status_with_metadata(
        self, workflow, sample_lead, mock_db_session
    ):
        ...
        await workflow._update_lead_status(
            sample_lead, LeadStatus.HANDED_OFF, metadata=metadata
        )

        assert sample_lead.status == LeadStatus.HANDED_OFF
        assert (
            sample_lead.metadata["handoff_timestamp"] == metadata["handoff_timestamp"]
        )
    ...
    @pytest.mark.asyncio
    async def test_update_lead_status_database_error(
        self, workflow, sample_lead, mock_db_session
    ):
...
class TestNotificationLogging:
    ...
    @pytest.mark.asyncio
    async def test_log_contractor_notification_success(
        self, workflow, qualified_lead, sample_contractors, mock_notification_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_log_customer_notification_success(
        self, workflow, qualified_lead, mock_notification_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_log_notification_failure(
        self, workflow, qualified_lead, sample_contractors, mock_notification_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_log_multiple_notifications(
        self, workflow, qualified_lead, sample_contractors, mock_notification_service
    ):
        """Test logging multiple notifications."""
        for contractor in sample_contractors:
            notification_result = {
                "message_id": f"msg_{contractor.id}",
                "status": "sent",
            }
            await workflow._log_notification(
                lead_id=qualified_lead.id,
        ...
        assert mock_notification_service.log_notification.call_count == len(
            sample_contractors
        )

    @pytest.mark.asyncio
    async def test_create_notification_record(
        self, workflow, qualified_lead, sample_contractors, mock_notification_service
    ):
...
class TestErrorHandlingAndRollback:
    ...
    @pytest.mark.asyncio
    async def test_rollback_on_email_failure(
        self,
        workflow,
        qualified_lead,
        sample_contractors,
    ...
    @pytest.mark.asyncio
    async def test_rollback_on_database_failure(
        self, workflow, qualified_lead, mock_db_session
    ):
    ...
    async def test_partial_failure_handling(
        ...
        with patch.object(
            workflow, "_find_matching_contractors", return_value=sample_contractors
        ):
    ...
    @pytest.mark.asyncio
    async def test_no_contractors_found_handling(
        self, workflow, qualified_lead, mock_db_session
    ):
        ...
        with patch.object(workflow, "_find_matching_contractors", return_value=[]):
            with patch.object(workflow, "_is_lead_qualified", return_value=True):
                with patch.object(
                    workflow, "_get_lead_by_id", return_value=qualified_lead
                ):
