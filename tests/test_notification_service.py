...
class TestEmailSending:
    ...
    @pytest.mark.asyncio
    async def test_send_email_success(self, notification_service, mock_smtp_server, sample_email_data):
        ...
        with patch('smtplib.SMTP') as mock_smtp_class:
            ...
            with patch.object(notification_service, '_render_template') as mock_render:
                ...
                with patch.object(notification_service, '_log_notification') as mock_log:
    ...
    @pytest.mark.asyncio
    async def test_send_email_smtp_connection_error(self, notification_service, sample_email_data):
    ...
    @pytest.mark.asyncio
    async def test_send_email_authentication_error(self, notification_service, mock_smtp_server, sample_email_data):
        """Test email sending with authentication error."""
        with patch('smtplib.SMTP') as mock_smtp_class:
            mock_smtp_class.return_value.__enter__.return_value = mock_smtp_server
            mock_smtp_server.login.side_effect = smtplib.SMTPAuthenticationError(535, "Authentication failed")
    ...
    @pytest.mark.asyncio
    async def test_send_email_recipient_error(self, notification_service, mock_smtp_server, sample_email_data):
    ...
    @pytest.mark.asyncio
    async def test_send_bulk_emails_success(self, notification_service, mock_smtp_server):
    ...
    @pytest.mark.asyncio
    async def test_send_bulk_emails_partial_failure(self, notification_service, mock_smtp_server):
...
class TestTemplateRendering:
    ...
    def test_render_template_success(self, notification_service):
        ...
        template_name = 'welcome'
        template_data = {'username': 'testuser', 'activation_link': 'https://example.com/activate/123'}
        
        with patch('jinja2.Environment') as mock_env_class:
            mock_env = Mock()
            mock_template = Mock()
            mock_template.render.return_value = '<html><body>Welcome testuser!</body></html>'
    ...
    def test_render_template_undefined_variable(self, notification_service):
        ...
        with patch('jinja2.Environment') as mock_env_class:
            mock_env = Mock()
            mock_template = Mock()
            mock_template.render.side_effect = jinja2.UndefinedError("'username' is undefined")
            mock_env.get_template.return_value = mock_template
            mock_env_class.return_value = mock_env
            notification_service._jinja_env = mock_env
...
class TestDatabaseOperations:
    ...
    @pytest.mark.asyncio
    async def test_log_notification_success(self, notification_service, sample_notification_record):
        ...
        notification_id = await notification_service._log_notification(**notification_data)
    ...
    async def test_log_notification_database_error(self, notification_service):
        ...
        notification_service.db.execute.side_effect = Exception("Database connection failed")
    ...
    @pytest.mark.asyncio
    async def test_get_notification_history(self, notification_service, sample_notification_record):
    ...
    @pytest.mark.asyncio
    async def test_get_failed_notifications(self, notification_service, sample_notification_record):
        ...
        failed_notifications = await notification_service.get_failed_notifications(max_retries=3)
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_retry_failed_notifications_success(self, notification_service, mock_smtp_server):
        ...
        with patch.object(notification_service, 'get_failed_notifications') as mock_get_failed:
            ...
            with patch('smtplib.SMTP') as mock_smtp_class:
                ...
                with patch.object(notification_service, '_render_template') as mock_render:
                    ...
                    with patch.object(notification_service, '_update_notification_status') as mock_update:
    ...
    @pytest.mark.asyncio
    async def test_retry_failed_notifications_max_retries_exceeded(self, notification_service):
        ...
        with patch.object(notification_service, 'get_failed_notifications') as mock_get_failed:
            ...
            with patch.object(notification_service, '_update_notification_status') as mock_update:
                result = await notification_service.retry_failed_notifications(max_retries=3)
                
                assert result['total_processed'] == 1
                assert result['successful_retries'] == 0
                assert result['failed_retries'] == 0
                assert result['max_retries_exceeded'] == 1
                mock_update.assert_called_with(1, 'permanently_failed', 'Maximum retry attempts exceeded')
    
    @pytest.mark.asyncio
    async def test_graceful_degradation_on_template_error(self, notification_service, sample_email_data):
    ...
    @pytest.mark.asyncio
    async def test_connection_pool_exhaustion_handling(self, notification_service, sample_email_data):
...
class TestAsyncOperations:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_email_sending(self, notification_service, mock_smtp_server):
        ...
        with patch('smtplib.SMTP') as mock_smtp_class:
            ...
            with patch.object(notification_service, '_render_template') as mock_render:
                ...
                tasks = [notification_service.send_email(**email_data) for email_data in email_tasks]
    ...
    async def test_rate_limiting(self, notification_service):
        ...
        with patch.object(notification_service, '_check_rate_limit') as mock_rate_limit:
            mock_rate_limit.side_effect = [True, True, False]  # Third call hits rate limit
    ...
    async def test_cleanup_old_notifications(self, notification_service):
        ...
        with patch.object(notification_service.db, 'execute') as mock_execute:
            ...
            deleted_count = await notification_service.cleanup_old_notifications(days_to_keep)
